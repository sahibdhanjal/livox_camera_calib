...
class Calibration {
 public:
  ros::NodeHandle nh_;
  ros::Publisher rgb_cloud_pub_ = nh_.advertise<sensor_msgs::PointCloud2>( "rgb_cloud", 1 );
  ros::Publisher init_rgb_cloud_pub_ = nh_.advertise<sensor_msgs::PointCloud2>( "init_rgb_cloud", 1 );
  ros::Publisher planner_cloud_pub_ = nh_.advertise<sensor_msgs::PointCloud2>( "planner_cloud", 1 );
  ros::Publisher line_cloud_pub_ = nh_.advertise<sensor_msgs::PointCloud2>( "line_cloud", 1 );
  ros::Publisher image_pub_ = nh_.advertise<sensor_msgs::Image>( "camera_image", 1 );
  ...
  void projection(
	  ...
	  const bool is_fill_img,
	  cv::Mat& projection_img );

  void calcLine(
	  const std::vector<Plane>& plane_list,
...
Calibration::Calibration(
  ...
  // canny threshold - edge size (ideal for our case was 12)
  // edge_minlen - minmum contour length to be added to edge PC
  // grey image - input
  // edge image - output of canny and contouring with appropriate thresholding
  // rgb_edge_cloud_ - edge cloud formed by edge image above
  cv::Mat edge_image;
...
void Calibration::colorCloud(
  ...
  cv::Mat distortion_coeff = ( cv::Mat_<double>( 1, 6 ) << k1_, k2_, k3_, k4_, p1_, p2_ );
  cv::Mat r_vec =
	  ( cv::Mat_<double>( 3, 1 ) << rotation_vector3.angle() * rotation_vector3.axis().transpose()[0],
...
void Calibration::edgeDetector(
	...
	cv::Mat& edge_img,
	pcl::PointCloud<pcl::PointXYZ>::Ptr& edge_cloud ) {
  cv::GaussianBlur( src_img, src_img, cv::Size( 5, 5 ), 0, 0 );
  ...
  if ( DEBUG ) {
	std::cout << "num edges > edge threshold: " << ctr << "  |   total num edges: " << contours.size() << std::endl;
	cv::imwrite( "/home/data/output/canny_edges.png", canny_result );
	cv::imwrite( "/home/data/output/edge_img_after_contours.png", edge_img );
  }
  ...
  if ( DEBUG ) {
	pcl::io::savePCDFileASCII( "/home/data/output/edge_pcd_cloud_after_edge_detector.pcd", *edge_cloud );
	std::cout << "Saved Edge Cloud PCD" << std::endl;
  }
}
  ...
  for ( size_t i = 0; i < pts_2d.size(); ++i ) {
	...
	if ( point_2d.x <= 0 || point_2d.x >= width_ || point_2d.y <= 0 || point_2d.y >= height_ ) {
	  ...
	  if ( projection_type == DEPTH ) {
		...
		float grey = depth_weight * depth / max_depth_ * 65535 + ( 1 - depth_weight ) * intensity / 150 * 65535;
		if ( image_project.at<ushort>( point_2d.y, point_2d.x ) == 0 ) {
		  image_project.at<ushort>( point_2d.y, point_2d.x ) = grey;
		  rgb_image_project.at<cv::Vec3b>( point_2d.y, point_2d.x )[0] = depth / max_depth_ * 255;
...
// 填补雷达深度图像
cv::Mat Calibration::fillImg( const cv::Mat& input_img, const Direction first_direct, const Direction second_direct ) {
  ...
  for ( int y = 2; y < input_img.rows - 2; y++ ) {
	for ( int x = 2; x < input_img.cols - 2; x++ ) {
	  if ( input_img.at<uchar>( y, x ) == 0 ) {
		...
		if ( ( current_depth - left_depth ) > 5 && ( current_depth - right_depth ) > 5 && left_depth != 0 &&
			 right_depth != 0 ) {
		  fill_img.at<uchar>( y, x ) = ( left_depth + right_depth ) / 2;
		} else if (
			( current_depth - up_depth ) > 5 && ( current_depth - down_depth ) > 5 && up_depth != 0 &&
			down_depth != 0 ) {
	...
	if ( kdtree->nearestKSearch( searchPoint, K, pointIdxNKNSearch, pointNKNSquaredDistance ) > 0 ) {
	  for ( int j = 0; j < K; j++ ) {
		...
		if ( distance < dis_threshold ) {
		  ...
		  if ( ( line_count % 3 ) == 0 ) {
			cv::line(
				connect_img, cv::Point( search_cloud->points[i].x, -search_cloud->points[i].y ),
				cv::Point( tree_cloud->points[pointIdxNKNSearch[j]].x, -tree_cloud->points[pointIdxNKNSearch[j]].y ),
				color, 1 );
		  }
		}
	  }
...
void Calibration::initVoxel(
  ...
  if ( DEBUG ) {
	pcl::io::savePCDFileASCII( "/home/data/output/voxel_map.pcd", test_cloud );
  }

  // assign points to each voxel here. Can save the voxel map by saving
  // each *(iter->second->cloud) as a PCD
  for ( auto iter = voxel_map.begin(); iter != voxel_map.end(); iter++ ) {
	// change cloud size from 20 to 50 and only then downsample
	if ( iter->second->cloud->size() > 50 ) {
	  down_sampling_voxel( *( iter->second->cloud ), down_sample_size_ );
	}
  }
}

// main crux of algorithm
void Calibration::LiDAREdgeExtraction(
  ...
  if ( DEBUG ) {
	std::cout << "Iterating through Voxels for plane & line extraction.." << std::endl;
  }

  int ctr = 0, num_planes_with_plane_list_gt2 = 0;
  for ( auto iter = voxel_map.begin(); iter != voxel_map.end(); iter++ ) {
	if ( iter->second->cloud->size() > plane_size_threshold_ ) {
	  ++ctr;
	  ...
	  std::cout << "Number of Planes for Voxel #" << ctr << " : " << plane_list.size() << std::endl;

	  // if condition not given, this will go to calcLine and get returned as the
	  // line_cloud_list is empty
	  if ( plane_list.size() < 2 )
		continue;

	  if ( DEBUG ) {
		std::string color_planner_cloud_name =
			"/home/data/output/color_planner_cloud_" + std::to_string( ctr ) + ".pcd";
		pcl::io::savePCDFileASCII( color_planner_cloud_name, color_planner_cloud );
		num_planes_with_plane_list_gt2++;
	  }

	  std::vector<pcl::PointCloud<pcl::PointXYZI>> line_cloud_list;

	  // line calculations from planes
	  calcLine( plane_list, voxel_size_, iter->second->voxel_origin, line_cloud_list );

	  std::cout << "Iterating through line cloud of size #" << ctr << " : " << line_cloud_list.size() << std::endl;
	  if ( DEBUG ) {
		for ( int i = 0; i < line_cloud_list.size(); i++ ) {
		  const auto& line_cloud = line_cloud_list[i];
		  std::string line_cloud_name =
			  "/home/data/output/line_cloud_" + std::to_string( ctr ) + "_" + std::to_string( i ) + ".pcd";
		  pcl::io::savePCDFileASCII( line_cloud_name, line_cloud );
		}
	  }
...
void Calibration::buildPnp(
  ...
  cv::Mat camera_matrix = ( cv::Mat_<double>( 3, 3 ) << fx_, s_, cx_, 0.0, fy_, cy_, 0.0, 0.0, 1.0 );
  cv::Mat distortion_coeff = ( cv::Mat_<double>( 1, 6 ) << k1_, k2_, k3_, k4_, p1_, p2_ );
  cv::Mat r_vec =
	  ( cv::Mat_<double>( 3, 1 ) << rotation_vector3.angle() * rotation_vector3.axis().transpose()[0],
		rotation_vector3.angle() * rotation_vector3.axis().transpose()[1],
  ...
  for ( size_t i = 0; i < search_cloud->points.size(); i++ ) {
	...
	if ( kdtree->nearestKSearch( searchPoint, K, pointIdxNKNSearch, pointNKNSquaredDistance ) > 0 ) {
	  for ( int j = 0; j < K; j++ ) {
		...
		if ( distance < dis_threshold ) {
		  cv::Point p_l_2d( search_cloud->points[i].x, -search_cloud->points[i].y );
		  cv::Point p_c_2d( tree_cloud->points[pointIdxNKNSearch[j]].x, -tree_cloud->points[pointIdxNKNSearch[j]].y );
		  if ( checkFov( p_l_2d ) ) {
			lidar_2d_list.push_back( p_l_2d );
			img_2d_list.push_back( p_c_2d );
		  }
...
void Calibration::loadImgAndPointcloud(
  ...
  for ( const rosbag::MessageInstance& m : view ) {
	if ( is_use_custom_msg_ ) {
	  livox_ros_driver::CustomMsg livox_cloud_msg = *( m.instantiate<livox_ros_driver::CustomMsg>() );	// message type
...
void Calibration::calcResidual(
	const Vector6d& extrinsic_params,
	const std::vector<VPnPData> vpnp_list,
	std::vector<float>& residual_list ) {
  residual_list.clear();
  Eigen::Vector3d euler_angle( extrinsic_params[0], extrinsic_params[1], extrinsic_params[2] );
  Eigen::Matrix3d rotation_matrix;
  rotation_matrix = Eigen::AngleAxisd( extrinsic_params[0], Eigen::Vector3d::UnitZ() ) *
					Eigen::AngleAxisd( extrinsic_params[1], Eigen::Vector3d::UnitY() ) *
					Eigen::AngleAxisd( extrinsic_params[2], Eigen::Vector3d::UnitX() );
  ...
  for ( size_t i = 0; i < vpnp_list.size(); i++ ) {
	...
	float rd = theta + k1_ * t3 + k2_ * t2 * t3 + k3_ * t2 * t2 * t3 + k4_ * t3 * t3 * t3;
	rd = rd / Ri;
...
void Calibration::calcCovarance(
	const Vector6d& extrinsic_params,
	const VPnPData& vpnp_point,
	const float pixel_inc,
	const float range_inc,
  ...
  Eigen::Vector3f base_vector1( 1, 1, -( direction( 0 ) + direction( 1 ) ) / direction( 2 ) );
  ...
  B << fx / p_c( 2 ), 0, fx * p_c( 0 ) / pow( p_c( 2 ), 2 ), 0, fy / p_c( 2 ), fy * p_c( 1 ) / pow( p_c( 2 ), 2 );
