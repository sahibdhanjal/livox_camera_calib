...
class pnp_calib {
 public:
  ...
  bool operator()( const T* _q, const T* _t, T* residuals ) const {
	...
	T rd = theta + distorT[0] * t3 + distorT[1] * t2 * t3 + distorT[2] * t2 * t2 * t3 + distorT[3] * t3 * t3 * t3;
	rd = rd / Ri;
	T xd = xo * rd;
	T yd = yo * rd;
  ...
  for ( int n = 0; n < 2; n++ ) {
	for ( int round = 0; round < 3; round++ ) {
	  ...
	  for ( int iter = 0; iter < max_iter; iter++ ) {
		...
		test_params << test_euler[0], test_euler[1], test_euler[2], calib_params[3], calib_params[4], calib_params[5];
		std::vector<VPnPData> pnp_list;
		calibra.buildVPnp(
			test_params, match_dis, false, calibra.rgb_egde_cloud_, calibra.plane_line_cloud_, pnp_list );
		...
		if ( cost < min_cost ) {
		  ...
		  calibra.buildVPnp(
			  calib_params, match_dis, true, calibra.rgb_egde_cloud_, calibra.plane_line_cloud_, pnp_list );
		  if ( DEBUG ) {
			cv::Mat projection_img = calibra.getProjectionImg( calib_params );
			cv::imshow( "Rough Optimization", projection_img );
			cv::waitKey( 50 );
			cv::imwrite( "/home/data/output/rough_optimization.png", projection_img );
		  }
		}
	  }
	}
...
int main( int argc, char** argv ) {
  ...
  calib_params << init_euler_angle( 0 ), init_euler_angle( 1 ), init_euler_angle( 2 ), init_transation( 0 ),
	  init_transation( 1 ), init_transation( 2 );
  ...
  distor << calibra.k1_, calibra.k2_, calibra.k3_, calibra.k4_, calibra.p1_, calibra.p2_;
  ...
  if ( DEBUG ) {
	...
	cv::waitKey( 50 );
	cv::imwrite( "/home/data/output/initial_extrinsic.png", init_img );
  }
  ...
  if ( DEBUG ) {
	...
	cv::waitKey( 50 );
	cv::imwrite( "/home/data/output/after_rough_extrinsic.png", test_img );
  }
  ...
  for ( dis_threshold = 30; dis_threshold > 10; dis_threshold -= 1 ) {
	...
	for ( int cnt = 0; cnt < 2; cnt++ ) {
	  if ( use_vpnp ) {
		calibra.buildVPnp(
			calib_params, dis_threshold, true, calibra.rgb_egde_cloud_, calibra.plane_line_cloud_, vpnp_list );
	  } else {
		calibra.buildPnp(
			calib_params, dis_threshold, true, calibra.rgb_egde_cloud_, calibra.plane_line_cloud_, pnp_list );
	  }
	  std::cout << "Iteration:" << iter++ << " Dis:" << dis_threshold << " pnp size:" << vpnp_list.size() << std::endl;

	  if ( DEBUG ) {
		cv::Mat projection_img = calibra.getProjectionImg( calib_params );
		cv::imshow( "Optimization", projection_img );
		cv::waitKey( 50 );
		cv::imwrite( "/home/data/output/optimization.png", projection_img );
	  }
	  ...
	  std::cout << "q_dis:" << RAD2DEG( opt_q.angularDistance( q ) ) << " ,t_dis:" << ( T - ori_t ).norm() << std::endl;
	}
	if ( !opt_flag ) {
	  break;
	}
  ...
  if ( DEBUG ) {
	...
	cv::waitKey( 50 );
	cv::imwrite( "/home/data/output/optimization_result.png", opt_img );
  }
